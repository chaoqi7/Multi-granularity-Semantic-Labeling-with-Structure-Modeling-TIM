...
def get_info(args):
    ...
    return {
        'node_feats': 13 if args.pc_attribs=='' else len(args.pc_attribs),
        'edge_feats': edge_feats,
        'classes': 6, #CHANGE TO YOUR NUMBER OF CLASS
        'inv_class_map': {0:'class_A', 1:'class_B'}, #etc...
    }

def preprocess_pointclouds(TANKER_PATH):
    """ Preprocesses data by splitting them by components and normalizing."""

    for n in ['train', 'test']:
        # pathP = '{}/parsed/{}/'.format(TANKER_PATH, n)
        # pathD = '{}/features/{}/'.format(TANKER_PATH, n)
        # pathC = '{}/superpoint_graphs/{}/'.format(TANKER_PATH, n)
        pathP = TANKER_PATH+'/parsed'+num+'/'+n+'/'
        pathD = TANKER_PATH+'/features'+num+'/'+n+'/'
        pathC = TANKER_PATH+'/superpoint_graphs'+num+'/'+n+'/'
        ...
        for file in os.listdir(pathC):
            ...
            if file.endswith(".h5"):
                ...
                elpsv = np.stack([ f['xyz'][:,2][:], f['linearity'][:], f['planarity'][:], f['scattering'][:], f['norm_zx'][:], f['curvature'][:], f['norm_zy'][:] ], axis=1)
                #elpsv = np.stack( [f['linearity'][:], f['planarity'][:], f['scattering'][:], f['norm_zx'][:], f['curvature'][:], f['norm_zy'][:]], axis=1)
                label = np.argmax(f['labels'][:,1:6].astype(np.int),axis=1)
                ...
                #elpsv[:,1:] -= 0.5
                rgb = 0*rgb
                P = np.concatenate([xyz, rgb, elpsv,label.reshape((label.shape[0],1))], axis=1)

                # P = np.concatenate([xyz, elpsv, label.reshape((label.shape[0], 1))], axis=1)
                ...
                with h5py.File(pathP + file, 'w') as hf:
                    for c in range(numc):
                        ...
                        if idx.size > 100000: # trim extra large segments, just for speed-up of loading time
                            ii = random.sample(range(idx.size), k=100000)
                            idx = idx[ii]
...
if __name__ == "__main__":
    ...
    preprocess_pointclouds(args.TANKER_PATH)              